...
if (-not $syncEnabled) {
  ...
  $push  = Join-Path $scripts 'sync-push.ps1'
  $psCmd = '-NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -File "' + $push + '" -Quiet'

  # Task Scheduler launching powershell.exe flashes a black console window for a
  # split second on every run - -WindowStyle Hidden cannot suppress it, because
  # the console is allocated before PowerShell reads the flag. wscript.exe is a
  # GUI host with no console at all, so run the identical command through a
  # one-line .vbs instead.
  #
  # The .vbs lives in ~\.claude, NOT in ~\.claude\timesheet-tools - that path is
  # a junction into this repo, so writing there drops an untracked file in git.
  $vbsPath = Join-Path $claudeDir 'timesheet-sync-hidden.vbs'
  $vbs = @"
' Runs the hourly timesheet sync with no console flash. Generated by install.ps1.
Set shell = CreateObject("WScript.Shell")
shell.Run "powershell.exe $($psCmd -replace '"','""')", 0, False
"@
  # wscript reads ANSI or UTF-16LE; Unicode is safe whatever the paths contain.
  Set-Content -Path $vbsPath -Value $vbs -Encoding Unicode
  # Earlier fix-terminal-popup runs wrote the launcher through the junction.
  $strayVbs = Join-Path $tools 'run-sync-hidden.vbs'
  if (Test-Path $strayVbs) { Remove-Item $strayVbs -Force; Write-Host "  removed stray $strayVbs" }
  ...
  Write-Host "  registered '$taskName' (hourly, hidden via $vbsPath)" -ForegroundColor Green
}
